...
jq -cn --arg ctx "Han pasado 7+ días (o nunca se ha corrido en este repo) desde la última pasada de la skill \`fewer-permission-prompts\`. En el primer momento natural de esta sesión, ejecútala (herramienta Skill, skill: \"fewer-permission-prompts\"). Al terminar -- haya añadido patrones nuevos a .claude/settings.json o no -- ejecuta SIEMPRE bash .claude/hooks/mark-permission-scan.sh \"<resumen corto>\": ese script ya comitea y sube el marcador él mismo (directo a la rama activa, sin crear una rama/PR nueva solo para esto), no hace falta que lo hagas tú aparte. Si la skill SÍ añadió patrones nuevos a .claude/settings.json, ese cambio de permisos sí sigue el flujo normal del repo (rama + PR en borrador, nunca fusionar sin pedirlo Angel) -- son dos cosas separadas." \
