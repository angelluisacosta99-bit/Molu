    ...
    const walk = (node) => {
      ...
      for (const ch of node.childNodes) {
        if (ch.nodeType === 3) s += ch.nodeValue;
          ...
          s += clean(ch.textContent) + " ";
        } else if (ch.matches?.("li, p, div, ul, ol")) {
          // Elementos de bloque dentro de un recuadro (la lista de frases de un ejercicio
          // de respuesta libre, por ejemplo): sin el salto se pegan todas en una línea.
          s += (s && !/\n$/.test(s) ? "\n" : "") + walk(ch);
