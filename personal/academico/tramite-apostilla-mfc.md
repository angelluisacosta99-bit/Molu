# Trámite de apostilla de títulos académicos rusos (MFC)

Estado del trámite de apostilla de los títulos académicos de Angel
Luis Acosta González obtenidos en Rusia (RUT MIIT), ante el MFC
(Многофункциональный центр) de Moscú. Sirve como memoria persistente
del proceso — actualizar este archivo cada vez que haya una novedad
(nueva respuesta del MFC, nuevo documento traducido, cambio de estado
en algún expediente), en vez de dejarla solo en el historial de una
conversación de Claude Code.

## Datos personales

- Nombre completo: Acosta González Angel Luis
- Nacionalidad: Cuba
- Pasaporte: M279418
- Dirección (Moscú): ul. Obraztsova, 22, 127055
- Email: angelluisacosta99@gmail.com
- Teléfono: +7 (985) 645-69-05

## Títulos académicos (RUT MIIT)

1. **Diploma de especialista** — serie 107704 № 0468100, registro n.º 4,
   expedido 30.06.2026. Especialidad 23.05.05 "Sistemas de aseguramiento
   de la circulación ferroviaria".
2. **Anexo al diploma** — serie 107704 № 0717708, registro n.º 4.
3. **Diploma de reconversión profesional** ("Predictive Analytics in
   Transport Tasks") — serie ППК № 003589, registro n.º 120, expedido
   01.07.2026.

## Solicitudes de apostilla ante el MFC (presentadas 03.07.2026)

| N.º de expediente | Documento | Estado |
|---|---|---|
| № 7638524892 | Diploma de especialista | ✅ Apostilla concedida — n.º **857967**, fecha 06.08.2026 |
| № 7638974873 | Diploma de reconversión profesional (ППК № 003589) | ❌ **Rechazada** (07.08.2026) — motivo alegado: RUT MIIT no respondió a tiempo al requerimiento del MFC. **Contradicho por RUT MIIT** (ver más abajo): la universidad sostiene que sí respondió en plazo, con referencia № 017/9760 del 15.07.2026. Se puede volver a solicitar sin pagar tasa, una vez el MFC de Moscú reciba/reconozca esa respuesta |

## Contactos relevantes

- **MFC (trámite original):** md@mfc.mos.ru — 119180, Moscú, ul.
  Bolshaya Yakimanka, 18. Tel. 8(495)777-77-77
- **Lugar para recoger el documento con apostille en papel:** 105082,
  Moscú, Tsentrosoyuzny per., 13, edif. 3. Tel. 8(495)777-77-77
- **Rosobrnadzor / FRDO:** frdo@obrnadzor.ru — tel. 8-800-775-44-22
- **MFC de la propia RUT MIIT:** spravka@rut-miit.ru — Responsable:
  Zolotova Maria Nikolaevna. Tel. +7 (915) 242-20-47 / 8(495) 274-02-74
  ext. 1111
- **Profesor de contacto (RUT MIIT):** Alexander Evgenievich —
  vanshin.ae@yandex.ru

## Gestiones pendientes

- **2026-09-04 — Hallazgo clave, contradicción entre RUT MIIT y el MFC
  de Moscú.** Angel escribió directamente al MFC de RUT MIIT
  (`spravka@rut-miit.ru`, el departamento indicado por Alexander el día
  anterior) explicando el rechazo. Respuesta del MFC de RUT MIIT: **la
  universidad sí respondió al requerimiento del MFC de Moscú, dentro de
  plazo — referencia de la respuesta № 017/9760, del 15.07.2026.** Esto
  contradice directamente el motivo con el que el MFC de Moscú rechazó
  la solicitud № 7638974873.
  - **Confirmado por dos documentos oficiales de Moscú, sin ambigüedad
    de expediente**: (1) el uveдомление de rechazo original **№
    7638974873/У-03 del 07.08.2026** (firmado por Лебедева Наталья
    Борисовна), que cita como base legal el **п. 21, пп. "ж" del
    Postановление Правительства РФ del 10.11.2023 № 1890**, y afirma
    textualmente que RUT MIIT "no envió la respuesta al MFC dentro del
    plazo establecido"; y (2) la respuesta a la obращение ciudadana
    **№ 02-12-5443/26 del 03.09.2026** (obращение № 57909473 / № 01-11-
    23421/26 del 07.08.2026, firmada por В.Н. Назарова), que **reafirma
    lo mismo, expediente por expediente**: № 7638974873 (ППК) rechazado
    por falta de respuesta a tiempo, № 7638524892 (especialista)
    aprobado. Es decir, **Moscú es inequívoco sobre a qué expediente se
    refiere** — no hay margen para que estén confundiendo los dos
    diplomas por su parte.
  - El uveдомление original también confirma el mecanismo de
    re-solicitud: si la universidad manda una respuesta informativa
    después del plazo, notifican la posibilidad de una nueva solicitud
    **sin pago de tasa estatal**.
  - **Pregunta abierta, pendiente de que RUT MIIT confirme**: si su
    referencia № 017/9760 (15.07.2026) corresponde de verdad al
    expediente № 7638974873 (ППК, rechazado) — en cuyo caso la
    respuesta se perdió en tránsito hacia el MFC de Moscú y hace falta
    reenviarla/certificarla — o si en realidad corresponde al otro
    expediente ya aprobado (№ 7638524892), y RUT MIIT nunca llegó a
    responder sobre el ППК. **Correo de aclaración enviado el
    2026-09-05** a `spravka@rut-miit.ru`, citando ambos documentos
    oficiales de Moscú (rechazo original № 7638974873/У-03 y respuesta
    a la obращение № 02-12-5443/26, esta última reenviada con su PDF
    adjunto). **Respuesta recibida el mismo día (09:40, Татьяна
    Соломатина): solo un acuse de recibo** ("передали на регистрацию" —
    lo pasaron para registrar) — **todavía sin responder la pregunta de
    fondo. Sigue a la espera de la respuesta sustantiva.**
- **2026-09-03** — Alexander Evgenievich (RUT MIIT) confirmó que la
  petición del MFC probablemente **se perdió dentro de la burocracia
  interna de RUT MIIT** (no fue el MFC quien falló). Pasó el contacto de
  un departamento concreto (`rut-miit.ru/depts/26467`, correo indicado
  en esa página — bloqueada por política de red en esta sesión, sin
  verificar el correo exacto) y se ofreció a ir en persona al MFC de la
  propia RUT MIIT si hiciera falta. **Angel ya escribió a ese
  departamento el mismo día — a la espera de respuesta.**
- **2026-09-03** — Alexander propuso una vía alternativa: verificó los
  datos de Angel en el **FIS FRDO** (registro federal ruso de
  documentos de educación/cualificación,
  `obrnadzor.gov.ru`/`school.kontur.ru` como guía de consulta) y
  confirma que el diploma **sí figura registrado como genuino**. Podría
  servir como prueba adicional ante el MFC o para la apostilla —
  **pendiente de que Angel lo use si el departamento no responde**.
- Esperar respuesta del MFC-RUT MIIT (spravka@rut-miit.ru) sobre el
  envío de la confirmación pendiente al MFC de Moscú — sigue en pie en
  paralelo a lo de arriba.
- Una vez el MFC reciba esa respuesta, presentar nueva solicitud de
  apostilla para el diploma de reconversión profesional (sin coste de
  tasa).
- Reclamación en curso vía obращение en mos.ru (portal ciudadano)
  referenciando ambos expedientes.

## Documentos ya traducidos al español (formato Word, para traductor jurado)

- Certificado de saldo bancario (Sberbank)
- 4 extractos de cuenta (Sberbank: 2 cuentas corrientes, cuenta de
  ahorro, depósito SberVklad)
- Extracto EGRIP (registro de empresario individual)
- 2 avisos de prórroga del MFC
- Certificado académico y certificado de finalización de estudios (RUT
  MIIT)
- Talón de notificación policial (denuncia de pérdida de pasaporte)
- Páginas de pasaporte (permiso de residencia temporal + visa)
- Notificación del MVD sobre el permiso de residencia temporal (RVP)
- Extracto bancario de TBank (cuenta de IP, 444 operaciones)

**Nota:** todas las traducciones son borradores; para el consulado se
necesita la firma de un traductor jurado oficial.

## Relación con la matrícula del máster (MUSI, USAL)

Este trámite es la causa raíz del riesgo de "matrícula condicionada"
registrado en
[`master-sistemas-inteligentes/admision-y-becas/becas-y-tramites-2026-2027.md`](../../master-sistemas-inteligentes/admision-y-becas/becas-y-tramites-2026-2027.md):
la matrícula de Angel en el MUSI está condicionada a la entrega del
título de grado, y esa entrega depende de que la apostilla del diploma
de especialista (ya concedida, n.º 857967) y la del diploma de
reconversión profesional (rechazada, pendiente de repetir) completen su
recorrido y lleguen a la universidad. Consultar ese archivo para el
plazo límite (22 de septiembre de 2026) y su impacto en la Beca de
Colaboración en Departamentos Universitarios.
