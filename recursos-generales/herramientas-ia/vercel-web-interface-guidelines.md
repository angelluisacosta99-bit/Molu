...
(MIT). Copia local del `README.md` original, obtenida el 2026-08-17, para
tenerla disponible sin depender de una petición web en cada sesión.
Actualizar manualmente si el repo original cambia de forma relevante — no
hay ningún hook que la sincronice sola.

**Para qué sirve aquí:** checklist adicional de calidad de interfaz (más
de 100 reglas concretas, no principios vagos) para usar junto con la
skill `impeccable` al revisar o pulir cualquier frontend de este repo
(ejercicios interactivos de `docencia-espanol/`, cualquier dashboard o
artifact). No sustituye a `impeccable` — la aportación de Vercel es una
...
## Vercel-specific (no aplican tal cual a este repo)

Preferencias de marca/producto de Vercel (voz activa, Title Case en
botones, placeholders concretos, etc.) — quedan fuera, no aportan a este
repo salvo como inspiración general de tono. La lista completa vive en la
fuente original si hace falta consultarla.

---

**Cómo usarlo en la práctica:** tras cualquier pase de `impeccable` sobre
una página nueva (ej. un ejercicio interactivo de `docencia-espanol/`),
repasar la sección "Interactions" y "Content" de arriba contra el HTML
resultante — son las dos secciones más aplicables a páginas estáticas
autocontenidas sin backend. "Performance" y "Forms" aplican sobre todo si
la página tiene envío de datos real (ej. el botón de WhatsApp/Telegram en
